...
public class SchemaParser {
    ...
    GeneratedClass requestWrapper;
    GeneratedClass responseHandler;

    GeneratedClass baseRequest;
    GeneratedClass baseResponse;


    public SchemaParser(File xmlSchema, File destinationDir, String targetPackage, String requestWrapper, String responseHandler, String baseRequest, String baseResponse) throws Exception {
        ...
        this.requestWrapper = codeGenerator.createClass(requestWrapper, null);
        this.responseHandler = codeGenerator.createClass(responseHandler, null);
        this.baseRequest = codeGenerator.createClass(baseRequest, null);
        this.baseResponse = codeGenerator.createClass(baseResponse, null);
    ...
    public void parse() throws Exception {
        ...
        logger.info("API Generator execution complete. Generated " + codeGenerator.generatedClasses.size() + " classes in " + totalTime + " milliseconds.");
    ...
    protected void processGroup(XSModelGroup modelGroup, ParseContext parseContext) throws Exception {
        ...
        for (XSParticle particle : modelGroup.getChildren()) {
            ...
            if(parseContext.subClass && 0 == index++) continue;
    ...
    private void addSimpleType(String name, XSSimpleType simpleType, ParseContext parseContext, boolean isAttribute) throws Exception {
        parseContext.currentClass.addCapability(
                name,
                requestWrapper,
                responseHandler,
                baseRequest,
    ...
    protected void processElement(XSElementDecl element, ParseContext parseContext) throws Exception {
        ...
        if (element.getType().isComplexType()) {
            ...
            parseContext.subClass = complexType.getBaseType().getTargetNamespace().equals(schema.getTargetNamespace());

            if(parseContext.subClass) {

                parseContext.currentClass = codeGenerator.createInterface(
